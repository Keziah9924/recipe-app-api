    ...
    transports: [
        new winston_1.transports.Console({
            ...
            format: winston_1.format.combine(winston_1.format.label({
                ...
                format: 'MMM-DD-YYYY HH:mm:ss',
            }), winston_1.format.printf((warn) => `${warn.level}: ${warn.label}: ${[warn.timestamp]}: ${warn.message}`), winston_1.format.colorize({
                all: true
            })),
        }),
        new winston_1.transports.Console({
            ...
            format: winston_1.format.combine(winston_1.format.label({
                ...
                format: 'MMM-DD-YYYY HH:mm:ss',
            }), winston_1.format.printf((console) => `${console.level}: ${console.label}: ${[console.timestamp]}: ${console.message}`), winston_1.format.colorize({
                all: true
            })),
        }),
        new winston_1.transports.Console({
            ...
            format: winston_1.format.combine(winston_1.format.label({
                ...
                format: 'MMM-DD-YYYY HH:mm:ss',
            }), winston_1.format.printf((error) => `${error.level}: ${error.label}: ${[
                error.timestamp,
            ]}: ${error.message}`), winston_1.format.colorize({
                all: true
            })),
        }),
        new winston_daily_rotate_file_1.default(warnOptions.file),
    ]
});
